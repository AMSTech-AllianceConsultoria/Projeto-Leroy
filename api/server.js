...
async function chamarEfdSpedBatchPuppeteer(env, caminhos) {
  ...
  const raw = await chamarTaxOneViaPuppeteerRaw(
    env,
    "POST",
    `/~1.0.0~/tax4b-efd-odata/OD_EFD.xsodata/$batch?portalInterceptorAppId=${appId}`,
    body,
    { "Content-Type": `multipart/mixed;boundary=${boundary}`, "Accept": "multipart/mixed" },
...
async function listarVariantesEfdSpedPuppeteer(env) {
  ...
  const retorno = await chamarTaxOneViaPuppeteer(
    env,
    "GET",
    `/~1.0.0~/tax4b-efd-js/variante/?portalInterceptorAppId=${appId}`,
    null,
    efdSpedBrowserKey(env)
  );
...
async function validarPeriodoEfdSpedPuppeteer(env, periodo, orgstr) {
  ...
  const retorno = await chamarTaxOneViaPuppeteer(
    env,
    "POST",
    `/~1.0.0~/tax4b-efd-js/EFD/validate-period-status/?portalInterceptorAppId=${appId}`,
    payload,
    efdSpedBrowserKey(env)
  );
...
async function gerarEfdSpedFiscalPuppeteer(env, periodo, orgstr, orgstrNome = "", varianteId = "", varianteCodigo = "", empresaEfd = "", filialEfd = "") {
  ...
  const resultado = await chamarTaxOneViaPuppeteer(
    env,
    "POST",
    `/~1.0.0~/tax4b-efd-js/EFD/?portalInterceptorAppId=${appId}`,
    payload,
    efdSpedBrowserKey(env)
  );
...
app.get("/api/:env/grupos/:grupoId/relatorios", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Filtro TaxOne obrigatório falhou. A lista HANA não será exibida para evitar relatórios extras. Causa:", err.message);
...
function montarCronExpression({ recorrencia, horario, diaSemana, diaMes }) {
  ...
  if (recorrencia === "mensal") {
    const dm = Number(diaMes);
    if (Number.isNaN(dm) || dm < 1 || dm > 5) {
      throw new Error("Dia do mês inválido. Para recorrência mensal, selecione 1º, 2º, 3º, 4º ou 5º dia de cada mês.");
    }
    return `${Number(minuto)} ${Number(hora)} ${dm} * *`;
  }
...
function calcularProximaExecucaoAgendamento(agendamento, referencia = new Date()) {
  ...
  if (agendamento.recorrencia === "mensal") {
    const dia = Number(agendamento.diaMes || 1);
    if (Number.isNaN(dia) || dia < 1 || dia > 31) return null;
    let proxima = new Date(ref.getFullYear(), ref.getMonth(), dia, hora, minuto, 0, 0);
    if (proxima <= ref) {
      proxima = new Date(ref.getFullYear(), ref.getMonth() + 1, dia, hora, minuto, 0, 0);
    }
    return proxima;
  }
...
async function executarAgendamento(agendamento) {
  ...
  try {
    ...
    registrarHistoricoExecucao({
      ...
      status: resultados.some(r => r.status === "ERRO") ? "CONCLUIDO_COM_ERRO" : "CONCLUIDO",
      periodo,
...
function carregarTarefasAgendadas() {
  ...
  for (const agendamento of agendamentos) {
    ...
    try {
      ...
      const task = cron.schedule(expression, () => executarAgendamento(agendamento), {
...
app.post("/api/agendamentos/:id/executar-agora", async (req, res) => {
  ...
  await executarAgendamento(agendamento);
  res.json({ status: "EXECUCAO_CONCLUIDA", agendamento_id: agendamento.id });
});
...
app.patch("/api/agendamentos/:id", (req, res) => {
  try {
    ...
    if (req.body.horario || req.body.recorrencia || req.body.diaSemana !== undefined || req.body.diaMes !== undefined) {
