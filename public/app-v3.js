console.log('Schedulle Reports UI v6 carregado - seleção em massa e dia útil mensal ativos');
...
async function loadModuleOptions(){
  ...
  try{
    if(modulo === 'tart'){
      ...
      const [estruturas, variantes] = await Promise.all([
        api(`/api/${env}/efd-sped/estruturas?mes=${mes}&ano=${ano}`),
        api(`/api/${env}/efd-sped/variantes`)
      ]);
      state.opcoes = estruturas;
...
function buildPayload(){
  ...
  if(modulo==='efd_sped'){ const el=$('input[name="orgstr"]:checked'); const varEl=$('#varianteSelect'); const opt=varEl?.selectedOptions?.[0]; payload.orgstr=el?.value; payload.orgstrNome=el?.dataset.nome || el?.value; payload.empresaEfd=el?.dataset.empresa || ''; payload.filialEfd=el?.dataset.filial || ''; payload.varianteId=varEl?.value || ''; payload.varianteCodigo=opt?.dataset.codigo || varEl?.value || ''; payload.varianteNome=opt?.dataset.nome || ''; }
  return payload;
}
