console.log('TaxOne Cockpit UI v17 carregado - atualização rápida de agendamentos');
...
async function refreshSchedulesOnly(showMsg=false){
  const btn = $('#refreshSchedulesBtn');
  ...
  try{
    ...
    renderDashboard();
    renderSchedules();
    if(showMsg) showAlert('Lista de agendamentos atualizada.');
  }catch(e){
    showAlert('Não foi possível atualizar os agendamentos: ' + e.message, 'erro');
  }finally{
    if(btn){ btn.disabled = false; btn.textContent = oldText || 'Atualizar lista'; }
  }
}
...
async function saveSchedule(e){
  ...
  try{
    ...
    for(const payload of payloads){
      const criado = await api('/api/agendamentos',{method:'POST',body:JSON.stringify(payload)});
      if(criado && criado.id) criados.push(criado);
    }
    if(criados.length){
      const ids = new Set(criados.map(a=>a.id));
      state.agendamentos = [...criados, ...state.agendamentos.filter(a=>!ids.has(a.id))];
      renderDashboard();
      renderSchedules();
    }
    showAlert(payloads.length === 1 ? 'Agendamento salvo com sucesso.' : `${payloads.length} agendamentos salvos com sucesso.`);
    $('#scheduleForm').reset(); initModuleSelect(); updateConditionalFields(); await loadModuleOptions();
    refreshSchedulesOnly(false);
...
function setView(id){ $$('.view').forEach(v=>v.classList.toggle('active', v.id===id)); $$('.nav-item').forEach(b=>b.classList.toggle('active', b.dataset.view===id)); $('#pageTitle').textContent = ({dashboard:'Visão geral',agendamentos:'Agendamentos',historico:'Histórico',diagnostico:'Diagnóstico'})[id] || 'TaxOne Cockpit'; }
...
$('#scheduleSearch').addEventListener('input', renderSchedules); $('#scheduleStatus').addEventListener('change', renderSchedules); if($('#refreshSchedulesBtn')) $('#refreshSchedulesBtn').addEventListener('click',()=>refreshSchedulesOnly(true)); $('#historySearch').addEventListener('input', renderHistory); $('#historyStatus').addEventListener('change', renderHistory);
