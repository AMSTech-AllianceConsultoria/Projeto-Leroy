console.log('Schedulle Reports UI v23 carregado - detalhes com itens processados');
...
function normalizarStatusHistorico(h){
  const raw = String(h?.status || h?.resultado?.status || h?.ultimo_status || '').trim().toUpperCase();

  // AVISO = execução disparada, mas houve falha apenas na consulta/listagem de status do TaxOne.
  // Isso não deve contar como falha operacional do relatório.
  if (raw.includes('AVISO') || raw.includes('WARNING') || raw.includes('CONCLUIDO_COM_AVISO')) return 'AVISO';
  if (raw.includes('CONCLUIDO_COM_ERRO') || raw.includes('ERRO') || raw.includes('ERROR') || raw.includes('FALH')) return 'ERRO';
  if (raw.includes('ENVIADO_AO_TAXONE') || raw.includes('PROCESSADO') || raw.includes('DISPARADO')) return 'PROCESSADO';
  if (raw.includes('CONCLUIDO') || raw.includes('OK') || raw.includes('SUCESS')) return 'SUCESSO';

  if (Array.isArray(h?.resultados) && h.resultados.length) {
    const temErroBloqueante = h.resultados.some(r => {
      const st = String(r?.status || '').toUpperCase();
      const erro = String(r?.erro || r?.detalhe || '');
      if (st.includes('AVISO') || st.includes('WARNING')) return false;
      if (!/ERRO|ERROR|FALH/i.test(st)) return false;
      return !erroTaxOneNaoBloqueante(erro);
    });
    const temAviso = h.resultados.some(r => /AVISO|WARNING/i.test(String(r?.status || '')) || erroTaxOneNaoBloqueante(String(r?.erro || r?.detalhe || '')));
    if (temErroBloqueante) return 'ERRO';
    if (temAviso) return 'AVISO';
    return 'SUCESSO';
  }

  return 'PENDENTE';
}
function erroTaxOneNaoBloqueante(msg){
  const texto = String(msg || '');
  return /tax4b-tart-js\/relatorio\//i.test(texto) ||
    /consultar.*status|status.*TaxOne|Filtro TaxOne|lista.*TaxOne|Internal server error/i.test(texto) && !/geracao_relatorio\/geracao/i.test(texto);
}
function formatarPeriodo(periodo){
  if (!periodo) return '-';
  if (typeof periodo === 'string') return periodo;
  if (typeof periodo === 'number') return String(periodo);
  if (typeof periodo === 'object') {
    if (periodo.mes && periodo.ano) return `${String(periodo.mes).padStart(2,'0')}/${periodo.ano}`;
    if (periodo.periodo && periodo.ano) return `${String(periodo.periodo).padStart(2,'0')}/${periodo.ano}`;
    if (periodo.dt_inicial && periodo.dt_final) return `${periodo.dt_inicial} até ${periodo.dt_final}`;
    if (periodo.dtInicial && periodo.dtFinal) return `${periodo.dtInicial} até ${periodo.dtFinal}`;
  }
  return '-';
}
function statusBadge(status, ativo){
  ...
  if (s.includes('AVISO') || s.includes('WARNING')) return '<span class="badge warn">Aviso</span>';
  if (s.includes('ERRO') || s.includes('ERROR') || s.includes('FALH')) return '<span class="badge erro">Erro</span>';
  if (s.includes('PROCESSADO') || s.includes('ENVIADO_AO_TAXONE') || s.includes('DISPARADO')) return '<span class="badge ok">Processado</span>';
...
async function loadSchedulesOnly(showMessage=false){
  try{
    const ag = await api('/api/agendamentos').catch(()=>[]);
    state.agendamentos = Array.isArray(ag) ? ag : [];
    renderDashboard();
    renderSchedules();
    if(showMessage) showAlert('Lista de agendamentos atualizada.');
  }catch(e){ showAlert(e.message || 'Falha ao atualizar agendamentos.', 'erro'); }
}
async function loadHistoryOnly(showMessage=false){
  try{
    const hist = await api('/api/agendamentos-historico').catch(()=>[]);
    state.historico = Array.isArray(hist) ? hist : [];
    renderDashboard();
    renderHistory();
    if(showMessage) showAlert('Histórico atualizado.');
  }catch(e){ showAlert(e.message || 'Falha ao atualizar histórico.', 'erro'); }
}
function renderDashboard(){
  const ativos = state.agendamentos.filter(a=>a.ativo !== false).length;
  const sucesso = state.historico.filter(h=>['SUCESSO','AVISO'].includes(normalizarStatusHistorico(h))).length;
  const erro = state.historico.filter(h=>normalizarStatusHistorico(h) === 'ERRO').length;
...
async function loadHealth(){ const box = $('#healthCards'); box.innerHTML = '<div class="health-item"><strong>Carregando...</strong><span>Consultando QAS e PRD.</span></div>'; const envs = await Promise.all(['qas','prd'].map(async env=>{ try { const r = await api(`/api/health/${env}`); return { env, ok:true, r }; } catch(e){ return { env, ok:false, e }; }})); box.innerHTML = envs.map(x=>`<div class="health-item"><strong>${x.env.toUpperCase()} ${x.ok ? '✅':'⚠️'}</strong><span>${x.ok ? 'Ambiente disponível' : escapeHtml(x.e.message)}</span></div>`).join(''); }
...
function renderHistory(){
  ...
  const st = ($('#historyStatus')?.value || '').toUpperCase();
  const all = state.historico || [];
  const okCount = all.filter(h=>['SUCESSO','AVISO'].includes(normalizarStatusHistorico(h))).length;
  const errCount = all.filter(h=>normalizarStatusHistorico(h) === 'ERRO').length;

  if($('#histOk')) $('#histOk').textContent = okCount;
  if($('#histErro')) $('#histErro').textContent = errCount;
  if($('#histTotal')) $('#histTotal').textContent = all.length;
  if($('#histUltima')) $('#histUltima').textContent = all.length ? fmtDate(all[0].fim || all[0].inicio || all[0].executado_em || all[0].data) : '-';

  let list = all.map((h, idx) => ({ h, idx }))
    .filter(({h})=>JSON.stringify(h).toLowerCase().includes(q));
  if(st) {
    list = list.filter(({h}) => {
      const status = normalizarStatusHistorico(h);
      return st === 'OK' || st === 'SUCESSO' ? status === 'SUCESSO' : status === 'ERRO';
    });
  }

  $('#historyBody').innerHTML = list.map(({h, idx})=>{
    const status = normalizarStatusHistorico(h);
    const periodo = formatarPeriodo(h.periodo || h.ultimo_periodo || h.competencia);
    return `<tr><td>${fmtDate(h.inicio || h.fim || h.executado_em || h.data)}</td><td>${escapeHtml(h.moduloNome || h.modulo || h.agendamento_nome || h.nome || '-')}</td><td><span class="badge env">${escapeHtml(String(h.env||h.ambiente||'-').toUpperCase())}</span></td><td>${escapeHtml(periodo)}</td><td>${statusBadge(status)}</td><td><button class="btn ghost" onclick="openDetail(${idx})">Ver detalhes</button></td></tr>`;
  }).join('') || '<tr><td colspan="6">Nenhum histórico encontrado.</td></tr>';
}
...
function labelValue(label, value, klass=''){
  return `<div class="detail-field ${klass}"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value || '-')}</strong></div>`;
}
function tempoExecucao(h){
  const ini = h?.inicio ? new Date(h.inicio) : null;
  const fim = h?.fim ? new Date(h.fim) : null;
...
function resumirResultadoItem(item, idx, statusExecucao=''){
  const nome = item?.nome || item?.relatorio?.nome || item?.orgstr || item?.empresa || item?.varianteCodigo || `Item ${idx + 1}`;
  const id = item?.id || item?.relatorio_id || item?.relatorio?.id || item?.varianteId || '';
  const rawItemStatus = String(item?.status || item?.resultado?.status || item?.ultimo_status || '').toUpperCase();
  let status = normalizarStatusHistorico(item);
  if ((statusExecucao === 'SUCESSO' || statusExecucao === 'AVISO') && (status === 'PENDENTE' || rawItemStatus.includes('ENVIADO_AO_TAXONE'))) {
    status = 'PROCESSADO';
  }
  const detalhe = item?.mensagem || item?.detalhe || item?.erro || item?.status || '';
  return `<div class="detail-result-item">
    <div>
      <strong>${escapeHtml(nome)}</strong>
      <small>${escapeHtml([id ? `ID ${id}` : '', item?.ambiente ? String(item.ambiente).toUpperCase() : '', item?.tipo ? `Tipo ${item.tipo}` : ''].filter(Boolean).join(' • '))}</small>
    </div>
    <div>${statusBadge(status)}</div>
    ${detalhe ? `<p>${escapeHtml(String(detalhe)).slice(0, 700)}</p>` : ''}
  </div>`;
}
function detalheAmigavel(h){
  if(!h) return '<div class="empty">Registro não encontrado.</div>';
  const status = normalizarStatusHistorico(h);
  const periodo = formatarPeriodo(h.periodo || { mes:h.mes, ano:h.ano });
  const resultados = Array.isArray(h.resultados) ? h.resultados : [];
  const itensHtml = resultados.length
    ? resultados.map((item, idx)=>resumirResultadoItem(item, idx, status)).join('')
    : '<div class="detail-empty">Nenhum item detalhado foi registrado para esta execução.</div>';
  const resumoErro = h.erro || h.mensagem || h.detalhe || h.resultado?.erro || h.resultado?.mensagem || '';
  return `
    <div class="detail-status-line">
      <div>
        <span class="eyebrow">Resultado</span>
        <h4>${escapeHtml(h.nome || h.agendamento_nome || h.moduloNome || 'Execução')}</h4>
      </div>
      ${statusBadge(status)}
    </div>
    <div class="detail-grid">
      ${labelValue('Módulo', h.moduloNome || h.modulo || '-')}
      ${labelValue('Ambiente', String(h.env || h.ambiente || '-').toUpperCase())}
      ${labelValue('Período', periodo)}
      ${labelValue('Início', fmtDate(h.inicio || h.data))}
      ${labelValue('Fim', fmtDate(h.fim))}
      ${labelValue('Duração', tempoExecucao(h))}
      ${labelValue('Total de itens', h.total || resultados.length || '-')}
      ${labelValue('ID do agendamento', h.agendamento_id || h.agendamentoId || '-')}
    </div>
    ${resumoErro ? `<div class="detail-message"><strong>Mensagem</strong><p>${escapeHtml(String(resumoErro)).slice(0, 1200)}</p></div>` : ''}
    <div class="detail-section-head"><h4>Itens processados</h4><span>${resultados.length} registro(s)</span></div>
    <div class="detail-results">${itensHtml}</div>
    <details class="technical-details">
      <summary>Ver JSON técnico</summary>
      <pre>${escapeHtml(json(h))}</pre>
    </details>`;
}
function openDetail(i){
  const registro = state.historico[i];
  $('#modalContent').innerHTML = detalheAmigavel(registro);
...
function setView(id){
  ...
  $('#pageTitle').textContent = ({dashboard:'Visão geral',agendamentos:'Agendamentos',historico:'Histórico',diagnostico:'Diagnóstico'})[id] || 'Schedulle Reports';
  if(id === 'historico') loadHistoryOnly(false);
  if(id === 'agendamentos') loadSchedulesOnly(false);
}
...
$('#refreshSchedulesListBtn')?.addEventListener('click', () => loadSchedulesOnly(true));
